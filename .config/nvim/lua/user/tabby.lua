...
local M = { "nanozuki/tabby.nvim", event = "VimEnter", dependencies = "nvim-tree/nvim-web-devicons" }
...
function M.config()
	...
	wk.register({
		t = {
			...
			t = { "<cmd>tab term<CR>i", "Open Terminal Tab" },
		},
		["="] = { "<cmd>tabn<CR>", "Goto Next Tab" },
		["-"] = { "<cmd>tabp<CR>", "Goto Prev Tab" },
	}, { prefix = "<leader>" })
	require("tabby").setup({})
	require("tabby.tabline").set(function(line)
		return {
			{ { tab_count(), hl = theme.head }, line.sep(" ", theme.head, theme.fill) },
